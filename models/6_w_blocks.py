...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.sa_heads(x) # apply the multi-head self attention to the input
        x = x + self.ffwd(x) # apply the feed-forward network to the output of the self attention layer
        return x
...
class BigramLanguageModel(nn.Module): # define the bigram language model class
    def __init__(self):
        ...
        self.blocks = nn.Sequential(*[Block(n_embed, n_head=4) for _ in range(4)]) # create a sequence of 4 Transformer blocks (for simplicity, we will use 4 blocks)
