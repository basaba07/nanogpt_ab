...
class BigramLanguageModel(nn.Module): # define the bigram language model class
    def __init__(self,vocab_size):
        super().__init__()
        self.token_embedding_table = nn.Embedding(vocab_size, vocab_size) # create an embedding layer that maps each token to a vector of size vocab_size (one-hot encoding)

    def forward(self, idx, targets=None):
        logits = self.token_embedding_table(idx) # get the logits by passing the input indices through the embedding layer
...
model = BigramLanguageModel(vocab_size).to(device) # create an instance of the bigram language model and move it to the specified device
